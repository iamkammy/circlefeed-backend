...
router.get('/test', (req,res) => {
    res.status(200).json({message: 'Circle API is working smoothly'});
})
    ...
    .then(savedUser => {
        ...
        .then(doMatch => {
            if(doMatch){
                ...
                res.status(200).json({status : 'success', message : 'Successfully signed in',user : {_id,name,email,pic,followers,following}, token});
            } else{
                return res.status(422).json({message : 'Invalid Email or password'});
            }
        })
